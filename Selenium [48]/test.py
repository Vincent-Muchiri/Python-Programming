...
while True:
    ...
    # TODO Check if the class of product not owned is 'enabled' or the price is affordable every minute and buy if the cookies are enough
    if time_elapsed >= time_interval_sec:
        # TODO Get the product
        next_product_elem = chrome_driver.find_element(By.CSS_SELECTOR, f"#product{next_product_num}")

        # TODO Get the price and compare it to the cookies earned
        # next_product_price = int(chrome_driver.find_element(By.CSS_SELECTOR, f"#productPrice{next_product_num}").text.replace(",", ""))
        # cookies_earned = int(chrome_driver.find_element(By.CSS_SELECTOR, f"#cookies").text.split()[0].replace(",", ""))
        # if cookies_earned >= next_product_price:
        ...
        if "enabled" in class_name:
            ...
            # TODO Check if the cookies left are enough to buy the previous item again
            if next_product_num != 0:
                previous_product_num = next_product_num - 1

                previous_product_elem = chrome_driver.find_element(By.CSS_SELECTOR, f"#product{previous_product_num}")
                # previous_product_price = int(chrome_driver.find_element(By.CSS_SELECTOR, f"#productPrice{previous_product_num}").text.replace(",", ""))
                # cookies_left = int(chrome_driver.find_element(By.CSS_SELECTOR, f"#cookies").text.split()[0].replace(",", ""))
                class_name = next_product_elem.get_attribute("class")
                if 'enabled' in class_name:
                    previous_product_elem.click()
