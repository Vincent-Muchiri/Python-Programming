...
recent_date = list(asset_data["Time Series (Daily)"].keys())[0]
before_recent_date = list(asset_data["Time Series (Daily)"].keys())[1]

# TODO Get the closing price of recent and the day before
recent_close = float(asset_data["Time Series (Daily)"][f"{recent_date}"]["4. close"])
before_recent_close = float(asset_data["Time Series (Daily)"][f"{before_recent_date}"]["4. close"])
